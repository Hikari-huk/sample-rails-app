...
class MicropostsSidebarTest < MicropostsInterface
  ...
  test "should display the right micropost count in sidebar" do
    get root_path
    assert_match "#{@user.microposts.count} microposts", response.body
  end

  test "should display correct micropost count for user with zero microposts" do
    log_in_as(users(:malory))
    get root_path
    assert_match "0 microposts", response.body
  end

  test "should display correct micropost count for user with one micropost" do
...
class ImageUploadTest < MicropostsInterface
  ...
  test "should upload an image with a micropost" do
    conttent = "This micropost has an image"
    img = fixture_file_upload('kitten.jpg', 'image/png')
    post microposts_path, params: { micropost: { content: conttent, image: img } }
